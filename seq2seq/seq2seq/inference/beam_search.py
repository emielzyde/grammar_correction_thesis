...
def length_penalty(sequence_lengths, penalty_factor):
  ...
  Returns:
    The length penalty factor, a tensor of shape [beam_size].
   """
...
def hyp_score(log_probs, sequence_lengths, config):
  ...
  #max_values = tf.math.argmax(log_probs, axis=1) 
  #one_hot_max_values =  tf.one_hot(indices=max_values, depth=tf.shape(log_probs)[1])
  #log_probs_augmented = log_probs + beta_2 * one_hot_max_values

  ### TOP NOISING 2 ###
  #Here, a penalty is only added to the overall top hypothesis
  #beta_3 = 30 #hyperparameter - need to tune this 
  #assert len(log_probs.shape) == 2, 'Log probs should have dimensionality of 2'
  #overall_max_values = tf.math.reduce_max(log_probs)
  #equal_max_values = tf.cast(tf.math.equal(overall_max_values, log_probs), dtype=tf.float32)
  #log_probs_augmented = log_probs + beta_3 * equal_max_values 

  ### RANK NOISING 
  #Here, a linearly increasing penalty is added, depending on the rank (maybe look at exponential)
  #beta_4 = 2 #hyperparameter - need to tune this 
  #sorted_values = tf.cast(tf.argsort(log_probs, axis = 1, stable=True), dtype = tf.float32) 
  #log_probs_augmented = log_probs + beta_4 * sorted_values
