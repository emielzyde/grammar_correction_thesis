...
export TEST_SOURCES=${HOMEDIR}/data/train/fce/snli_out.txt   # these correct sentences will be corrupted
export TEST_PREDS_DIR=${HOMEDIR}/data/NMT
mkdir -p $TEST_PREDS_DIR
export TEST_PREDS_PREFIX=${TEST_PREDS_DIR}/SNLI_TS   # fce-ts-$TEMP-$MODEL.txt is the file that will be written
export TEMP=0.05

export MODEL=87000
export TEST_PREDS=${TEST_PREDS_PREFIX}-${TEMP}
python3 -m bin.infer \
  --tasks "
    - class: DecodeText
      params:
