...
def txt_to_error_detection_tsv(source_txt, target_txt, main_label='i'):
    ...
    tsv_data.data_list = data_list
    tsv_data.propagate_list()
    return tsv_data, corpus_gleu, corpus_bleu, overall_correct, overall_incorrect, average_incorrect, average_correct, sentences_incorrect, sentences_total
...
def txts_to_tsv_wrapper(correct_sents_file, corrupted_sents_files, output_tsv_file, min_num_errors, max_num_errors,
    ...
    for corrupt_file in corrupted_sents_files:
        ...
        beam_data_tsv_1, corpus_gleu, corpus_bleu, correct_counter, incorrect_counter, avg_incorrect, avg_correct, sents_incorrect, sents_total = txt_to_error_detection_tsv(
            corrupt_file, correct_sents_file, main_label)
        data_tsv.merge_tsv(beam_data_tsv_1)

    #assert sents_total == len(data_tsv.data_list), 'Number of sentences should be equal'
    print('Incorrect tokens: ', incorrect_counter)
    print('Correct tokens: ', correct_counter)
    print('% Incorrect tokens: ', incorrect_counter / (incorrect_counter + correct_counter))
    ...
    print('Numbber of incorrect sentences', sents_incorrect)
    print('Percentage of incorrect sentences', sents_incorrect/len(data_tsv.data_list))
    print()

    if remove_duplicates:
        pre_removal = len(data_tsv.data_list)
        print('Removing duplicates...')
        data_tsv.remove_duplicates()
        post_removal = len(data_tsv.data_list)
        print('Number of sentences:', len(data_tsv.data_list))
        perc_duplicates = (post_removal-pre_removal)/pre_removal
        print('Percentage of duplicates: ', perc_duplicates)
...
def beam_search_wrapper_sample():
    correct_sents_file = '/Users/emielzyde/Downloads/round_translation_japanese_new.txt'
    corrupted_sents_file = '/Users/emielzyde/Downloads/wronging/data/NMT/japanese-11-69000.txt'
    output_tsv_file = '/Users/emielzyde/Downloads/japanese_beamsearch.tsv'
    beams = 11
    keep_beams = 3
    min_num_errors = 0
    max_num_errors = 5
...
def wrapper_sample():
    ...
    correct_sents_file = '/Users/emielzyde/Downloads/round_translation_korean.txt'
    corrupted_sents_file = ['/Users/emielzyde/Downloads/wronging/data/NMT/korean_argmax_2-87000.txt']
    output_tsv_file = '/Users/emielzyde/Downloads/korean_argmax_2.tsv'
